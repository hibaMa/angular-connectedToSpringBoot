...
export class AddRequestComponent implements OnInit {
  ...
  fillProductArray(selectedProject){
    let selectedProjectID = selectedProject.value;
    this.projectsArray.map((project)=>{
      if(project.id==selectedProjectID){
        this.productsArray=project.product;
      }
    });
  ...
  ngOnInit() {
    ...
    this.dataBaseService.getMediaTypes().subscribe(
      (mediaTypes) => {
        this.mediaArrayTypes = mediaTypes;
      }
    );
  ...
  initSubmitData(): void {
    ...
    this.sliderService.chosenPressesArray.map((press) => {
    ...
    this.submitRequesData = {
      ...
      hub:1,//hub id
      description: '',
      mediaTypes: submitMediaTypeArray,//ids
      ...
      components: submitComponentArray,//ids
